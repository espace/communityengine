...
class User < ActiveRecord::Base
  ...
  def self.find_active(options = {:limit => 10})
    commented_on = Comment.find(:all, :limit => 3, :include => :recipient, :conditions => "users.avatar_id is not null and users.featured_writer = false", :order => "comments.created_at desc").collect{ |c| c.recipient }.uniq
    posters = Post.find(:all, :limit => 3, :include => :user, :conditions => "users.avatar_id is not null and users.featured_writer = false" ,:order => "posts.published_at DESC").collect{ |p| p.user }.uniq
  ...
  def self.find_by_activity(options = {})
    ...
    activities = Activity.since(options[:since]).find_by_sql("select * , small_activities.count as activities_count from(
      select activities.user_id , max(activities.id) as activity_id ,  count(*) as count from activities  LEFT JOIN users ON users.id = activities.user_id
       #{options[:require_avatar] ? ' where users.avatar_id IS NOT NULL' : nil}  group by activities.user_id )  as small_activities JOIN users on small_activities.user_id  = users.id order by small_activities.count limit #{options[:limit] }"
      )
    
    
#    activities = Activity.since(options[:since]).find(:all, 
 #     :select => '*, count(*) as count', 
  #    :group => " activities.user_id ,  #{table_columns_seperated_by_commas('Activity')} ,  #{ table_columns_seperated_by_commas('User') } " , 
